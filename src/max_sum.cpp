...
T searchForward(std::list<int> &contiList, T beginIter, T endIter) {
  ...
  for (; curIter != endIter; ++curIter) {
    ...
    if (sum > 0) {
      contiList.insert(std::cend(contiList), std::cbegin(tempHold),
                       std::cend(tempHold));
...
int searchFromCenter(const Iter &center, const Iter &begin, const Iter &end) {
  ...
  auto curBackwardIter =
      std::reverse_iterator(center); // the reverse iterator will return the
                                     // previous element of normal iterator
...
int maxSequence(const std::vector<int> &arr) {
  ...
  for (auto curIter = std::begin(arr); curIter != std::cend(arr); ++curIter) {
    solCandidates.push_back(
        searchFromCenter(curIter, std::cbegin(arr), std::cend(arr)));
  }
  return *std::max_element(std::cbegin(solCandidates),
                           std::cend(solCandidates));
}
